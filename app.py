...
def ensure_spotify_authenticated():
    ...
    Stores token_info in st.session_state['token_info'] and creates
    st.session_state['spotify_client'] and st.session_state['current_user'].
    If not authenticated, shows a login link and stops execution.
    """
    # Use environment variables (Streamlit Cloud: set these in Secrets)
    CLIENT_ID = os.getenv("SPOTIPY_CLIENT_ID")
    CLIENT_SECRET = os.getenv("SPOTIPY_CLIENT_SECRET")
    REDIRECT_URI = os.getenv("SPOTIPY_REDIRECT_URI")

    if not CLIENT_ID or not CLIENT_SECRET or not REDIRECT_URI:
        st.error("Spotify credentials (SPOTIPY_CLIENT_ID / SPOTIPY_CLIENT_SECRET / SPOTIPY_REDIRECT_URI) are not set.")
        st.stop()
    ...
    sp_oauth = SpotifyOAuth(
        ...
        scope=SCOPE,
        show_dialog=True,   # force login dialog so each user chooses their account
        cache_path=None     # important: do NOT use a shared file cache
    )

    # 1) If we already have token_info stored in session, ensure it's valid
    token_info = st.session_state.get("token_info")
    if token_info:
        try:
            # If token_info looks like a dict with 'access_token'
            if hasattr(sp_oauth, "is_token_expired") and sp_oauth.is_token_expired(token_info):
                # refresh
                refreshed = sp_oauth.refresh_access_token(token_info.get("refresh_token"))
                # refresh_access_token usually returns a dict similar to token_info
                st.session_state["token_info"] = refreshed
                token_info = refreshed
        except Exception:
            # if refresh fails, clear token and restart login flow
            st.session_state.pop("token_info", None)
            token_info = None

    # 2) If there is no token_info in session, check if Spotify redirected back with a code
    if not token_info:
        query_params = st.experimental_get_query_params()
        if "code" in query_params:
            code = query_params["code"][0]
            try:
                token_response = sp_oauth.get_access_token(code)
            except TypeError:
                # some versions expect get_access_token(as_dict=False) or other signature
                token_response = sp_oauth.get_access_token(code=code)
            except Exception as e:
                st.error(f"Spotify token exchange failed: {e}")
                st.stop()

            # token_response may be a dict with access_token or a string; normalize
            if isinstance(token_response, dict):
                token_info = token_response
            else:
                # If spotipy returns access_token string, wrap minimal dict
                token_info = {"access_token": token_response}

            st.session_state["token_info"] = token_info
            # clear query params so reloading doesn't re-run exchange
            st.experimental_set_query_params()
        else:
            # No token and no code: show login link and stop
            auth_url = sp_oauth.get_authorize_url()
            st.markdown("### 🔐 Please log in with Spotify to continue")
            st.markdown(f"[**Log in with Spotify**]({auth_url})")
            st.stop()

    # 3) At this point we should have token_info
    ...
    # Create a Spotipy client bound to this user's token and store in session state
    try:
        ...
        st.error(f"Failed to create Spotify client: {e}")
        st.stop()
